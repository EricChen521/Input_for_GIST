...
"""
    this python script will require a aligned lignad pdb file and output the gist.in file for running gist
"""
...
class Point:
    #STORES THE X Y AND Z COORDINATES
        ...
        def Distance(firstpoint, secondPoint):
            d = sqrt(((firstpoint.x-secondPoint.x)**2)+ ((firstpoint.y-secondPoint.y)**2)+ ((firstpoint.z-secondPoint.z)**2))
            return d
        def length(Self):
...
def printCentroid():
    ...
    ligand_coordinate=open("ligand.center","w+")
    ligand_center="{0:6.3f} {1:6.3f} {2:6.3f}".format(x,y,z)
    ligand_coordinate.write(ligand_center)
    ligand_coordinate.close()

    # Wirte gist.in file for runing gist 
    f_gist=open("gist.in","w+")
    f_gist.write("parm ../receptor.prmtop\n")
    f_gist.write("trajin ../md10.nc\n")
    f_gist.write("strip :Na+,Cl-\n")
    f_gist.write("gist gridspacn 0.5 gridcntr "+ligand_center+" griddim 48 48 48\n")
    f_gist.write("go")
    f_gist.close()
